...
function tf_contactform_shortcode($atts){
    ...
    if($form_exists){
        ...
        $out .='<h3 id="header_message">'.urldecode($form['header_message']).'</h3><div id="form_messages" class="submit_message" ></div>';
        ...
        $out.='<div class="comment-form"><form id="contactForm" action="" method="post" class="ajax_form contactForm" name="contactForm">';
        ...
        foreach($form['input'] as $form_input){
            ...
            if($is_preview && $input['type'] == 'select') $form_input['ewidthpx'] -=2;
        ...
        <img id="sending2" src="'.$surse.'" alt="Sending" style="margin-left:6px; margin-bottom:-6px; display: none;" /></div></div>
        </form></div>';
...
function text($input,$form_input){
    return "<div class='row field_text alignleft infieldlabel ".$form_input['classes']."' style='".$form_input['floating']."'>
                ...
                <input type='text' id='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'style='width:".($form_input['ewidthpx'])."px;' class='".$input['properties']['class']."' name='".TF_THEME_PREFIX.'_'. trim($form_input['shortcode'])."'/>
            </div>";
}

function textarea($input,$form_input){
    return "<div class='row field_textarea infieldlabel ' style='".$form_input['floating']."'>
...
function checkbox($input,$form_input){
    ...
    $output = "<div class='row alignleft input_styled checklist ".$form_input['classes']."' style='width:".($form_input['ewidthpx']+12)."px;".$form_input['floating']."' >
                <label for='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "'>".$form_input['label_text']."</label>
                <div class='rowCheckbox'><input class='".$input['properties']['class']."' style='width:15px;' type='checkbox' name='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "' id='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "' value='".$form_input['label']."'" . $checked . "/></div>
            </div>";
    return $output;
}
